...
- **Дедупликация** — `AiDeduplicationService` поверх `BasicDeduplicationService`, на эмбеддингах:
  1. дешёвая эвристика (нормализованный URL/видео, Jaccard заголовков/тем, детекция «развития темы») — если уверена, доверяем ей;
  2. эмбеддинг кандидата (`PolzaEmbeddingClient`, модель `Ai:Polza:EmbeddingModel`) сравнивается косинусом с сохранёнными эмбеддингами свежих постов (поле `Post.EmbeddingJson`, считается при создании поста). cosine ≥ 0.90 → дубль; так ловятся одна новость из разных изданий и на разных языках;
  3. только «серая зона» похожести (0.82–0.90) подтверждается одним chat-вызовом. То есть дорогого chat-вызова на каждый кандидат нет — только дешёвый эмбеддинг и сравнение в памяти.

  Пороги (0.90 / 0.82) — стартовые, могут потребовать подстройки на реальных данных. _Next step:_ pgvector + ANN-индекс вместо in-memory косинуса при росте объёма.
